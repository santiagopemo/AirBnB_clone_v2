...
class TestDBStorage(unittest.TestCase):
    ...
    def test_all_cls(self):
        ...
        self.assertIn(self.state, obj.values())
    ...
    def test_save(self):
        ...
        db = MySQLdb.connect(user="hbnb_test",
                             ...
                             db="hbnb_test_db")
        cursor = db.cursor()
        cursor.execute("SELECT * FROM states WHERE name = 'Virginia'")
        query = cursor.fetchall()
        self.assertEqual(1, len(query))
        self.assertEqual(st.id, query[0][0])
        self.storage._DBStorage__session.delete(st)
        self.storage.save()
        cursor.close()
